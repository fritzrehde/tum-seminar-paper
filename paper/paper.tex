...
In this tree structure, each object is \emph{inside} its owner.
...
The object \emph{engine} is \emph{inside} the object \emph{car} because \emph{engine} is owned by \emph{car}.
...
Even the original Flexible Alias Protection model, which these ownership types are based on, imposed the restriction "that the only objects that can access [an object] are the object that owns it and other objects inside that [object]" \cite{flexible-alias-protection}.
...
Most importantly, these three rules imply that any external reference to an object is only possible through its owner.
This is also examplified in Figure \ref{fig:owners-as-dominators} where the object \emph{world} is trying to access the object \emph{engine}.
This is not allowed in the owners-as-dominators model, as \emph{world} is neither a direct owner, a sibling or inside of \emph{engine}.
...
% TODO: I could easily link this back to rust
...
% TODO: check if rust implements all rules from dominators or modifiers

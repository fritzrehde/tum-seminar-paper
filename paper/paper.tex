...
In most ownership-type variants, the heap is divided into different regions called \empg{ownership contexts}.
...
%     \node[object] (A) {a};
%     \node[object] (C) [right=of A,yshift=1cm] {c};
%     \node[object] (B) [below=of A] {b};
%     \node[draw,thin,inner sep=2mm,fit=(A.west) (B) (B) (A.west)] {};

%     \draw[->,thick] (A.south) to node[left] {owns} (B.north);
%     \draw[->,thick,dashed] (C.south) to node[right] {no access} (B.north);
%   \end{tikzpicture}

%   \caption{Owners-as-modifiers \cite{ownership-types-survey}}
% \end{figure}
...
\begin{figure}
  \begin{tikzpicture}[object/.style={rectangle, draw, very thick, minimum size=20}]
    ...
    \draw[->,thick] (World.south) to node[left] {owns} (A.north);
    \draw[->,thick] (World.south) to node[right] {owns} (C.north);
    \draw[->,thick] (A.south) to node[left] {owns} (B.north);
    \draw[->,thick,dashed] (C.south) to node[right] {no access} (B.north);
  \end{tikzpicture}

...
Given the two objects \emph{a} and \emph{b}, \emph{a} is \emph{outside} \emph{b} if \emph{b} is the same object as \emph{a} or \emph{b} is \emph{owned} by \emph{a}, transitively.
...
\begin{figure}
  \begin{tikzpicture}[object/.style={rectangle, draw, very thick, minimum size=20}]
    ...
    % TODO: make green darker with:
    % \definecolor{mynicegreen}{RGB}{102,252,102}

    \draw[->,thick,dashed,green] (Car) edge[bend left] node[right] {owner} (Engine);
    % \draw[->,thick,dashed,green] (Engine) edge[bend left] node[left] {inside} (Car);
    \draw[->,thick,dashed,green] (Engine) edge[bend left=25] node[left] {inside} (World);
    \draw[->,thick,dashed,green] (Car) edge node[above] {sibling} (Driver);
...
The validity of each valid reference is proven by each of the rules defined above.

\begin{enumerate}
  \item The object \emph{car} owns the object \emph{engine}, meaning that the former can validly reference the latter according to the first rule.
  \item The objects \emph{car} and \emph{driver} are siblings, meaning that they can validly reference each other according to the second rule.
  \item The object \emph{world} is outside the object \emph{engine} because \emph{engine} is transitively owned by \emph{world} via \emph{car}.
  Therefore, according to the third rule, \emph{engine} can validly reference \emph{world}.
\end{enumerate}
...
An implication of this model is that any external reference to an object is only possible through its owner.
This is also examplified in Figure \ref{fig:owners-as-dominators} through the object \emph{world} trying to access \emph{engine}.
This is not allowed in the owners-as-dominators model.
Instead, \emph{world} can only reference \emph{engine} through the \emph{engine}'s owner \emph{car}.

Its strictness can be seen as both an advantage and disadvantage of the owners-as-dominators model.
% TODO: reference ownership survey better, as this is nearly copied
The advantage is that model provides a simple, clear and strong guarantee that allows for reasoning about various properties of the code.
% TODO: provide example of advantages
However, it also makes programming more difficult by not allowing common idioms that involve aliasing.
% TODO: example of such idiom that isn't representable


% TODO: Add rust section on taking ownership of fields inside type
% Struct Car {
%   engine: Engine,
% }

% let car = Car::new(Engine::new());
% let driver = Driver::new();


% Abstract. Ownership types were devised nearly 15 years ago to provide
% a stronger notion of protection to object-oriented programming languages.
% Rather than simply protecting the fields of an object from external ac-
% cess, ownership types protect also the objects stored in the fields, thereby
% enabling an object to claim (exclusive) ownership of and access to other
% objects. Furthermore, this notion is statically enforced by now-standard
% type-checking techniques.
% Originating as the formalisation of the core of Flexible Alias Protec-

% IMPORTANT TODO: question: can an object access owned children transitively? How is it in Rust (if someone asks me this question)?
...
% TODO: this is the same as dominators
With owners-as-dominators, every path from the root to a given object must pass through that object's owner.

...
\begin{figure}
  \begin{tikzpicture}[object/.style={rectangle, draw, very thick, minimum size=20}]
    ...
    \node[object] (A) [below=of World] {a};
    \node[object] (C) [right=of A] {c};
    \node[object] (B) [below=of A] {b};

    \draw[->,thick] (World) to node[left] {owns} (A);
    \draw[->,thick] (World) to node[right] {owns} (C);
    \draw[->,thick] (A) to node[left] {owns} (B);
    \draw[->,thick,dashed] (C) to node[right] {invalid access} (B);
  \end{tikzpicture}

  \caption{Owners-as-modifiers}
...
\begin{figure}
  \begin{tikzpicture}[object/.style={rectangle, draw, very thick, minimum size=20}]
    ...
    \draw[->,thick,dashed] (Car) to node[above] {valid} (Driver);
    \draw[->,thick,dashed] (World) edge[bend right=45] node[left] {invalid} (Engine);
  \end{tikzpicture}

  \caption{Owners-as-modifiers \cite{flexible-alias-protection}, solid lines indicate "owns"}
  \label{fig:owners-as-modifiers2}
\end{figure}
...
Figure \ref{fig:owners-as-modifiers} provides an example of an invalid reference from object \emph{c} to \emph{b}, as there exists no direct link in the ownership hierarchy.
% TODO: what does this implicate further?

It is the strictest discipline.
...
% IMPORTANT TODO: question: can an object access owned children transitively?

% Advantages: useful for reasoning about properties of code.

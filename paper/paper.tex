...
% TODO: clarify that reference and alias is used synonymously in this paper
\paragraph{Memory safety issue example}

% The following describes an example in which ownership types can prevent a memory security issue.
...
% The initial contents of the vector \emph{vec} are stored in a buffer in memory.
...
% This is possible because both pointers were aliasing: an action through a pointer (v) will in general also affect all its aliases (vptr).
...
% The fact that \emph{vptr} is now a dangling pointer becomes a problem in the last line, where we load from vptr.
Finally, trying to access the value the dangling pointer \emph{vptr} points to will cause a use-after-free bug.
% Such runtime bugs are very hard to track down.
% The main problem here is that such use-after-free bugs cannot be detected at compile time.

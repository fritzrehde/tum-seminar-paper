...
% \subsection{From ownership types to Rust}

% These early ownership type models carry some flaws with them. (lack features to be actually used)
% For example, in a system using the owners-as-dominators approach, one cannot programmatically create iterators.

% We can link certain owners-as-modifiers concepts to concepts from Rust.
% Firstly, siblings...
...
Immutable references can be seen as read-only references and are acquired using the \verb|&| operator.
...
Rust also supports mutable references, which are created with the \verb|&mut| keyword and allow the borrowed value to be modified.
...
% The programmer must only annotate lifetimes explicitly when there is ambiguity in the relationship between the lifetimes of multiple references.
...
% Lifetimes answer the question: just how long is temporary?
...
% TODO: format so spaces align properly
\begin{lstlisting}[language=Rust]
let mut v = vec![10, 11];

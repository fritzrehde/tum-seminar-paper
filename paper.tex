...
println!("x: {}, y: {}", x, y);
\end{lstlisting}
% Since the \emph{String} value can only have one owner, it is \emph{moved} into \emph{y}, which will become its new owner.
...
% The following example illustrates that the ownership of stack-based values is not transferred.
% \begin{lstlisting}
% let x = 5;
% let y = x;
% y = y + 1;

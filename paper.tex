...
The primitive data types (e.g. signed 32 bit integers \emph{i32}) are immutable by default and stored on the stack since their sizes are small and known at compile time.
...
Rust supports the immutable string literals \emph{\&str}, which are stored on the stack, and the type \emph{String}, which is stored as a vector of bytes, heap allocated, and therefore growable. \cite{rust-by-example}

The \emph{String} type in Rust is a good example of such a type.
\begin{lstlisting}
...
% This, however, applies only to data types that are stored on the heap.

In the following piece of Rust code, one variable is assigned to another.
\begin{lstlisting}
let x: &str = "hello world";
let y: &str = x;
\end{lstlisting}
Since their sizes are known at compile time, they are simply copied into a new variable instead of moved.

...
% TODO: source
In addition to providing the memory safety that ownership types provide, ownership transfers can also reduce redundant copy operations.
...
% TODO: bad sentence
% The following piece of code shows a \emph{String} value being \emph{moved} from one variable into another.
The heap-allocated \emph{String} type in Rust provides a good example to demonstrate the transfer of ownership of values.
\begin{lstlisting}
let x: String = String::from("hello world");
let y: String = x;
println!("x: {}, y: {}", x, y);
\end{lstlisting}
% TODO: go into detail on how strings/heap types are stored on heap.
...
\subsubsection{Stack-based values}

First, one must differentiate between values that are stored on the stack and those stored on the heap.
% TODO: leave out immutable, adds no meaning to sentence
In Rust, primitive data types (e.g. signed 32 bit integers \emph{i32}) are immutable by default and stored on the stack, since their sizes are small and known at compile time.

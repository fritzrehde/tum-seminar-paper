...
The concept of ownership types provides one of the solutions aiming to elimate many of the described memory-related bugs.
...
\subsection{Flexible Alias Protection}
\label{sec:flexible-alias-protection}
...
One such flavour is the owners-as-dominators invariant \cite{ownership-types-survey}, where the heap is structured as a tree with each object being inside its owner.
...
A last flavour of ownership types is the owners-as-ombudsmen approach \cite{ownership-types-survey}, which allows idioms that do not adhere to the strong topological structure of a tree structure to be implemented.
% TODO: examples
...
If anything was retrieved from other sources, it will be cited as such.
...
\footnote{In some code snippets, the main function is omitted for sake of simplicity}
...
% However, this rule only holds as long as the scope of a mutable reference to the same value does not overlap with the scopes of the immutable references.
...
% The emphasis on the scope of references when only allowing multiple references with scopes that overlap is very important.
...
In summary, programming languages based on ownership types can eliminate many memory-related issues by enforcing strict rules on the existence and behaviour of references to values.
Furthermore, ownership types are integral to the Rust programming.
In comparison to more commonly used low-level programming languages like C, Rust introduces many memory-related rules that are strictly enforced by the compiler at compile time, allowing for strong runtime performance and omitting the need for many manual, rigorous and error-prone memory reference checks that programers used to have to perform.
Even though the Rust compiler forces developers to think more intentially about how they deal with memory through ownership, they will receive helpful compile-time error messages instead of crashes or undefined behaviour at runtime if they do something wrong.

It will be interesting to see how Rust's development will continue in the future.
Given its rise in popularity as a safe and performant programming language using strict ownership types, the question arises whether ownership types might also be incorporated into more new or existing programming languages in the future.
